...
\begin{definition}
Pokaži, da so gostote svetlobnega toka, pri katerih postane nelinearen 
...
\label{eq:chijk}
\end{equation}
Zaradi simetrije vpeljemo poenostavljen zapis, pri katerem prvi indeks 
prepišemo $(x = 1, y = 2, z = 3)$, zadnja dva indeksa pa združimo, podobno, 
kot smo naredili pri elektro-optičnem pojavu (razdelek~\ref{chap:EO}).
...
\end{equation}
Primerjajmo gornjo trditev s tabelo~(\ref{table:chi}). V tabeli
...
in spekter izhodne svetlobe. Intenzitete izhodnih žarkov niso v merilu.}
\label{fig:nl2}
\end{figure}
...
Navadna valovna enačba ne velja za opis pojavov pri velikih 
intenzitetah vpadnih valovanj, saj pride do pojava nelinearne polarizacije. Valovanje 
opišemo z nelinearno valovno 
...
Nelinearne valovne enačbe na splošno ne znamo rešiti in zatečemo k približkom.
...
ju lahko obravnavamo ločeno. Ni sicer nujno,
...
Dobimo\footnote{Spomnimo, da je 
$\chi^{(2)}:\mathbf{e}_{n}\,\mathbf{e}_{m}$ notranji produkt
tenzorja z enotskima vektorjema polarizacije, 
...
zberemo člene pri $\omega_3 = \omega_1 + \omega_2$. Zapišemo
\begin{equation}
...
in govorimo o frekvenčnem podvajanju\index{Optično frekvenčno podvajanje}. Gre za
najpreprostejši in tudi najpomembnejši nelinearni optični pojav. 
...
Uporabili smo splošne enačbe in privzeli, da je 
...
Izkoristek pretvorbe v frekvenčno podvojeno valovanje je torej velik le pri zelo majhnih vred\-no\-stih 
...
\begin{definition}
\label{deplet}
Pokazali smo, da gostota svetlobnega toka frekvenčno podvojenega valovanja $j_{2\omega}$ 
pri $\Delta k = 0$  narašča sorazmerno s kvadratom 
dolžine kristala (enačba~\ref{8.10}). Takšna odvisnost velja le, 
dokler je $j_{2\omega}$ bistveno manjša od $j_{\omega}$ oziroma $A_3 \ll A_1, A_2$. 
...
\caption{Izkoristek pretvorbe v frekvenčno podvojeno valovanje pri $\Delta k = 0$.
...
pri čemer smo lomnim količnikom pripisali frekvenco.
...
rišemo presek ploskve lomnega količnika z ravnino, določeno z optično osjo in valovnim
...
z modro krožnico, ki ustreza valovanju s podvojeno frekvenco. Pri kotu 
...
S slike~(\ref{fig:dk} c in d) razberemo, da obstaja še en primer, pri 
...
določen s kotom širjenja svetlobe glede na smer optične osi 
$\vartheta_m$ (enačbi~\ref{8.13} in \ref{8.13a}). Zaradi simetrije
kristala to pomeni, da obstaja cel stožec smeri, v katerih se faze ujemajo.
...
Valovna dolžina osnovnega snopa naj bo 1064~nm. 
...
\label{SHGGauss_3}
\end{align}
s pripadajočim sistemom kompleksno konjugiranih enačb. Pri tem je 
\begin{equation}
\kappa=\frac{\chi_{ef}}{c_0} \sqrt{\frac{\omega_{1}\omega_{2}\omega_{3}}{n_{1}n_{2}n_{3}}},
\label{8.20}
\end{equation}
s črtico pa smo označili odvajanje po $z$. Gornji sistem enačb je očitno
...
\caption{Shematski prikaz valovanj pri optičnem parametričnem ojačevanju}
\label{fig:opa2}
\end{figure}
...
\caption{Normirani intenziteti ojačenega žarka in dodatnega nedejavnega žarka, ki nastane zaradi ohranitve
...
~A_{10}.
\end{equation}
Pokaži še, da so gornje enačbe v limitnem primeru, ko se faze ujamejo in je $\Delta k = 0$,
...
Gornji izračun kaže, da optično parametrično ojačevanje svetlobe pri prehodu skozi kristal ni prav veliko
...
polju obstaja v vsaki snovi. V njem nastopa jakost električnega polja v tretji potenci
\boxeq{eq:nl3P}{
...
$10^{-22}~\si{\metre^2/\volt^2}$. Na splošno ima 81 različnih neodvisnih komponent, to
...
v produktu $\mathbf{E}\mathbin \mathbf{E}\mathbin\mathbf{E}$ 
dvakrat nastopa nekonjugirani del, enkrat pa konjugirani. To se zgodi na tri
načine, zato nastopajo trije enaki členi. Sledi
\begin{equation}
...
potuje.\index{Susceptibilnost!nelinearna, efektivna}
...
velikostnih redov, $n_2$ pa je lahko tudi negativen (tabela~\ref{table:chi3}). 
...
\caption{Shema metode vzdolžnega premika. Z -- zaslonka, D -- detektor, $n_2$ - optično
nelinearno sredstvo in $f$ leča.}
\label{fig:zscan}
\end{figure}
\end{remark}
...
\label{8.83}
\end{equation}
Izrazimo še gostoto svetlobnega toka z amplitudo jakosti električnega polja 
\begin{equation}
\nabla_{\bot}^{2}\psi+
...
\label{8.83a}
\end{equation}
Preden se lotimo reševanja gornje enačbe, vpeljemo še
\begin{equation}
...
\label{8.89}
\end{equation}
Gostota svetlobnega toka je neodvisna od koordinate $z$ in je sorazmerna kvadratu amplitude polja 
(enačba~\ref{8.89})
\boxeq{8.89a}{
j_{s}(x,z)= j_0 \frac{1}{\cosh^2(\eta x)}.
}\index{Gostota energijskega toka}
Ker $j\neq j(z)$, pomeni, da sunek ohranja svojo obliko ko se širi vzdolž osi $z$.
...
\end{equation}
pri čemer je $\omega_0$ osrednja krožna frekvenca svetlobnega sunka in $k_0 = \omega_0 \tilde{n}/c_0$. Vpeljemo še
$\Omega =\omega - \omega_0$
\begin{equation}
...
razvijemo v Taylorjevo vrsto okoli osrednje krožne frekvence $\omega_0$ do tretjega člena. Sledi
\begin{equation}
...
frontami, vendar potuje novo valovanje v nasprotni smeri od prvotnega. Novo valovanje je tako,
...
pri fazni konjugaciji pa zapis valovanja in njegova reprodukcija \index{Hologram}
potekata sočasno. 
\end{remark}
Koordinatni sistem izberemo tako, da se signalno valovanje širi v smeri $z$. Zapišemo ga  
\begin{equation}
...
ali v približku tudi Gaussov snop (b). Ko vpade na navadno zrcalo (zgoraj), se njegova divergenca
ohranja in žarek se še naprej razširja. Na fazno konjugiranem zrcalu se krogelni val po odboju spet
...
Račun poenostavimo še s predpostavko, da sta črpalna vala $E_{1}$
...
\label{8.100}
\end{equation}
Pri tem je $n^2\,\omega^{2}/c_0^{2}=k^{2}$, $P_{\textrm{NL}}$ pa je po enačbi~(\ref{eq:nl3P})
...
pri katerem se energija vpadnih fotonov (in z njo frekvenca svetlobe) ohranja, pride
...
svetlobe, $\nu_v$ pa vibracijsko frekvenco, v drugem primeru pa $\nu_{as}=\nu_0+\nu_v$.
...
\label{srs:X}
\end{equation}
Pri tem je $X$ koordinata, $\gamma$ koeficient dušenja, $m$ masa in $F$ zunanja sila.
...
\label{srs:F}
\end{equation}
Jakos električnega polja smo zapisali kot povprečje, saj so optične frekvence 
...
V meter dolgem odseku optičnega vlakna pride tako pri gostoti svetlobnega toka $10^{10}~\si{\watt/\meter^2}$ 
do faktorja ojačenja $1,08$, na vlaknu dolžine $20~\si{\metre}$ pa do faktorja $5$.
...
velikih intenzitetah lahko pride do nelinearnih pojavov. Oglejmo si nekaj najpomembnejših.
...
\index{Brillouinovo sipanje!stimulirano} do odboja vpadne svetlobe na 
...
\begin{enumerate}
\item 
Samo-fazna modulacija ({\it SPM -- Self-Phase Modulation}). Različni deli
...
in slika~\ref{fig:optsoliton}). \index{Samo-fazna modulacija}
...
\item
Navzkrižna fazna modulacija ({\it CPM -- Cross-Phase Modulation}).\index{Navzkrižna
fazna modulacija} Ko po vlaknu
...
valovanj pri dodatnih frekvencah (vsotah in razlikah frekvenc obstoječih valovanj). To se pojavi
