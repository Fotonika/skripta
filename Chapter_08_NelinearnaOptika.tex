...
\label{eq:chijk}
\end{equation}
Zaradi simetrije lahko vpeljemo poenostavljeni zapis, pri katerem prvi indeks 
...
razlikujemo zaradi možnosti dveh različnih polarizacij. Izhodno valovanje 
ima frekvenco $\omega_{3}=2\omega$ in govorimo o frekvenčnem 
podvajanju\index{Frekvenčno podvajanje}. Gre za
...
Izkoristek pretvorbe v frekvenčno podvojeno valovanje je torej velik le pri  majhnih vred\-no\-stih 
...
in $\omega_{1}\pm2\omega_{2}$ in pri treh vpadnih frekvencah vse
...
\label{8.100}
\end{equation}
pri čemer je $n^2\,\omega^{2}/c_0^{2}=k^{2}$. $P_{\textrm{NL}}$ je po enačbi~(\ref{eq:nl3P})
