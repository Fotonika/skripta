...
Fresnelov približek\index{Fresnelov uklon} (glej razdelek~\ref{FFuklon}). 
...
\end{equation}
pri čemer je $k=n\omega/c_{0}$ valovno število. Z $n$ smo označili lomni količnik
sredstva, po katerem se valovanje širi. Rešitev iščemo v obliki
...
\label{eq:phiphi}
\end{equation}
Vstavimo izraza za $q$ (enačba~\ref{eq:alpha}) in $\phi$ (enačba~\ref{eq:phiphi}) v nastavek za $\psi$ (enačba~\ref{eq:gaussov-snop-nastavek}):
\begin{equation}
...
polja\index{Rayleighovo območje|see{Območje bližnjega polja}}. Zaradi simetričnosti rešitve
...
\begin{equation}
P = \frac{1}{2}\frac{\pi w_0^2}{2}\varepsilon_0 c_0 E_0^2.
\end{equation}
\end{naloga}
...
Da so tudi ti snopi rešitve obosne valovne enačbe, se 
prepričamo, če izraz (enačba~\ref{eq:Gauss-Hermitevi}) vstavimo v obosno valovno enačbo\index{Obosna valovna enačba} (enačba~\ref{eq:obosna-valovna-enacba})
...
Pravimo, da Laguerre-Gaussovi snopi nosijo tirno vrtilno 
...
\begin{figure}[ht]
\centering
\includegraphics[width=9truecm]{slike/03_Laguerre_faza.png}
...
\end{equation}
pri čemer je $m$ zaradi zahteve po enoličnosti rešitev
...
\ref{fig:Prehod-Gaussovega-snopa}). Vpeljemo parametra:
\begin{equation}
...
$z$ je usmerjena v desno, gledamo pa referenčno glede na lego grla vsakega posameznega snopa. 
...
geometrijske optike (enačbe \ref{eq:MABCD1}, \ref{eq:MABCD2} in \ref{eq:Mdf}). 
...
\begin{table}[ht]
 \centering
  ...
  Opis prehoda & Skica & Matrika za prehod \\ \hline   
      Prehod skozi prostor za $d$ & \parbox[c]{3cm}{\def\svgwidth{25mm}\input{slike/03_matrika_d.pdf_tex}} & 
      $\begin{bmatrix} 1 & d\\  0 & 1 \end{bmatrix}$ \\ \hline

      Prehod skozi mejo dveh snovi & \parbox[c]{3cm}{\def\svgwidth{25mm}\input{slike/03_matrika_n.pdf_tex}} & 
      $\begin{bmatrix} 1 & 0\\ 0 & \frac{n_{1}}{n_{2}} \end{bmatrix}$ \\ \hline
      
      Prehod skozi ukrivljeno mejo $R>0$ & \parbox[c]{3cm}{\def\svgwidth{25mm}\input{slike/03_matrika_nR.pdf_tex}} & 
      $\begin{bmatrix} 1 & 0\\ \frac{(n_{1}-n_{2})}{n_{2}R} & \frac{n_{1}}{n_{2}} \end{bmatrix}$ \\ \hline
      
      Prehod skozi konveksno lečo $f>0$ & \parbox[c]{3cm}{\def\svgwidth{25mm}\input{slike/03_matrika_f.pdf_tex}} & 
      $\begin{bmatrix} 1 & 0\\ -\frac{1}{f} & 1 \end{bmatrix}$ \\ \hline
      
      Odboj na konkavnem zrcalu $R>0$ & \parbox[c]{3cm}{\def\svgwidth{25mm}\input{slike/03_matrika_R.pdf_tex}} & 
