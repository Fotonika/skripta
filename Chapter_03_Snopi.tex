...
Polmer odprtine naj bo $a$, območje bližnjega polja označuje $b$, $2\vartheta$ pa 
...
nastavek v Helmholtzevo enačbo (enačba~\ref{eq:valovna-enacba-hh})
...
Zapišimo še divergenco snopa v velikih oddaljenostih od grla. Polovični kot širjenja je
\begin{equation}
...
Pokaži, da je svetlobna moč v Gaussovem snopu 
enaka 
...
Razlika je v fazi, ki je odvisna tudi od $n$ in $m$
\begin{equation}
...
$L_{1}^{l}(x) = -x+l+1$, 
$L_{2}^{l}(x) = x^2/2-(l+2)x+(l+2)(l+1)/2$ ..., faza pa je 
...
\end{equation}
Rešitve so s faznim faktorjem pomnožene Besslove funkcije
\begin{equation}
\psi_m(r, \varphi)=J_{m}(k_{\perp}r)e^{im\varphi},
\end{equation}
kjer je $J_{m}$ Besslova funkcija reda $m$ in $m$ celo število. Za
...
optiki.\footnote{Glej npr. G. R. Fowles, {\it Introduction to Modern Optics}, druga izdaja, Dover Publications (1975).}
...
geometrijske optike (\ref{eq:MABCD1}, \ref{eq:MABCD2} in \ref{eq:Mdf}). Na splošno
...
\begin{table}[t!]
 \centering
  ...
  Opis prehoda & Skica & Matrika za prehod \\ \hline   
      ...
      Prehod skozi ukrivljeno mejo $R>0$ & \parbox[c]{3cm}{\def\svgwidth{3cm}\input{slike/03_matrika_nR.pdf_tex}} & 
