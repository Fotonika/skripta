...
vodnika in z zeleno plašč. Žarek se na meji med sredico in plaščem totalno odbija.}
\label{fig:vodnik}
\end{figure}
...
odvisna od krožne frekvence in pride do disperzije\index{Disperzija}. Grupna ali skupinska
...
Tipičen enorodovni vodnik ima tako debelino $a\lesssim 5~\si{\micro\meter}$, medtem ko je
...
\label{10BS}
\end{equation}
kjer je prečna komponenta valovnega vektorja enaka
\begin{equation}
...
Tudi LP$_{21}$ rodove, ki nastanejo kot kombinacija HE$_{31}$
...
To vodi do rezultata\index{Optično vlakno!število rodov}
\begin{equation} 
N = \frac{4 V^2}{\pi^2}.
\end{equation}
...
\begin{remark}
 ...
 z enako hitrostjo. V realnem vlaknu  
...
izogniti, vendar jo lahko pri dani valovni dolžini približno izničimo z materialno~(slika~\ref{fig:MatVal}). 
...
\label{9.72}
\end{equation}
Pri tem za dolžino sunka $2\sigma$ velja enaka zveza kot za premer 
Gaussovega snopa (enačba~\ref{eq:w})
\boxeq{9.73}{
\left(2\sigma\right)^{2}=\left(2\sigma_{0}\right)^{2}\left(1+\left(\frac{\zeta}{\zeta_{0}}\right)^{2}\right).
}
Tu je $2\sigma_{0}$ trajanje sunka pri $\zeta=0$, to je na začetku,
...
ki ga bomo spoznali pri obravnavi nelinearne optike (slika~\ref{fig:chirp}\,a). \index{Čirikanje}
...
razdalje je tako potreben razmeroma močen signal, ki ne sme biti premočen,
...
so podobni stanjem delca v konstantnem zunanjem električnem polju (desno). Podobno
...
sunek se ojači. Gre za povsem enak postopek ojačevanja svetlobe, kot ga poznamo iz 
...
\caption{Bočna sklopitev s prizmo (a) in s periodično strukturo (b)}
\label{fig:coupler}
\end{figure}
...
bodo tako postale odvisne od vzdolžne koordinate $z$.
...
vodnika različna, v drugem primeru (b) sta vodnika enaka in pride do popolnega prenosa.}
\label{fig:foscil}
\end{figure}
