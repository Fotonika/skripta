...
celo število. Pravimo, da vsak $N$ določa en rod valovanj v vodniku. 
...
Zapišimo še nekaj o hitrosti valovanja v vodniku.
...
plasti prozornega dielektrika z lomnim količnikom $n_1$ in plašča (ali okolice) 
z lomnim količnikom $n_2$.
...
pogoja  (enačbi~\ref{eq:rps1} in \ref{eq:rps2}) hkrati izpolnjena. Izraza za robna pogoja delimo, tako da se amplitude pokrajšajo, in dobimo sekularno 
...
\vglue-6truemm
\begin{remark}
Če ne prej, je bralec ob sliki~\ref{fig:TESodi} zagotovo opazil podobnost s kvantnim 
...
\label{fig:TEsec}
\end{figure}
Število presečišč krožnice  
s krivuljami v prvem kvadrantu določa število lastnih rodov v vodniku in lega presečišča
...
so izmenično sodi in lihi, pri čemer je osnovni rod vedno sodi. 
...
Ocenimo še število lastnih rodov v vodniku. Iz slike~\ref{fig:TEsec} je razvidno, da je 
...
vseh šest komponent električnega in magnetnega polja 
valovanja različnih od nič in vsi rodovi
imajo tudi komponento polja v smeri $z$. Take rodove 
imenujemo hibridni rodovi in jih 
označimo s HE, če je $E_z$ razmeroma velik ali vsaj 
primerljiv z $E_r$ in $E_\varphi$, 
oziroma z EH, če je $H_z$ po velikosti primerljiv s 
$H_r$ in $H_\varphi$ ali večji od njiju. 

Privzamemo, da se lomna količnika sredice in plašča 
le malo razlikujeta. Tudi v tem 
...
\right \} \, J_0(k_s r),
\end{equation}
saj ima dve možni smeri polarizacije. Na splošno rodovi HE$_{1m}$ ustrezajo 
rodovom LP$_{0m}$. 
...
razvoja Besslovih funkcij za velike argumente.\footnote{~Glej npr. C. R. Pollock, {\it Fundamentals of Optoelectronics}, Irwin (1995).} Približna ocena vključuje 
...
$\nabla\cdot{\bf D}=0$ polje ne more imeti povsod iste smeri; 
za natančnejši račun bi morali zapisati enačbo za
...
\begin{equation}
X'' - \frac{k_0^2 \Delta^2}{a^2}\,X\,x^2 - \lambda_1 X = 0
\label{eq:XY}
\end{equation}
in
\begin{equation}
...
\end{equation}
Pri tem sta $\lambda_1$ in $\lambda_2$ konstanti. 
...
\begin{naloga}
Uporabi nastavek (enačba~\ref{eq:GH}) in pokaži, da reši enačbo~(\ref{eq:XY}). 
Pri tem si pomagaj z 
...
enačb ustreza en lastni rod v vlaknu.
...
\label{9.62f}
\end{equation}
Osnovni sunek naj bo približno monokromatičen 
s krožno frekvenco $\omega_{0}$. Ta naj bo dovolj 
...
\end{remark}
\vglue-3truemm
Računa ni treba ponavljati, namesto tega kar v izrazu za Gaussove snope 
...
$b$, ki ustreza krivinskemu radiju $R$. Oba parametra sta seveda odvisna od $\zeta$, tako kot sta parametra $w$ in $R$ odvisna od $z$ (slika~\ref{fig:Gausstau}). 
\begin{figure}[ht]
\centering
\def\svgwidth{100truemm} 
...
Na podlagi analogije zapišemo obliko podaljšanega 
Gaussovega sunka\index{Gaussov sunek}:
\begin{equation}
...
Naj bo vpadni sunek svetlobe Gaussove oblike $E = 
\psi(x,y) e^{-at^2-i \omega_0 t}$.
Pokaži, da je ustrezna funkcija $\tilde{A}(\Omega)$ oblike:
\begin{equation}
...
\end{figure}
\vglue-4truemm
Naj na par vzporednih uklonskih mrežic vpada 
ravni val pod kotom $\alpha$, ki se odbija
...
\begin{remark}
Postopek kompenzacije disperzije se uporablja za pridobivanje zelo močnih in zelo
...
vpadna gostota svetlobnega toka. Če se po enem
...
določeni s termičnimi fluktuacijami. Pri optičnih zvezah tako glavna 
omejitev niso izgube, ampak je to popačitev signala zaradi disperzije.
...
Do zdaj smo privzeli, da je vlakno povsem ravno 
in da so mejne ploskve valovnega vodnika vzporedne. 
...
Če ponovno naredimo primerjavo
...
\caption{Bočna sklopitev svetlobe v optični vodnik s prizmo (a) in periodično strukturo (b)}
\label{fig:coupler}
\end{figure}
...
Imejmo dva enorodovna vodnika z lomnima količnikoma sredice $n_1$ in $n_2$ ter enako 
...
\begin{naloga}
 ...
 da med parametri enačb velja naslednja zveza, pri čemer je $K = \sqrt{K_{12}K_{21}}$ in 
...
vodnika različna, v drugem primeru (b) pa sta vodnika in prenos je popoln.}
\label{fig:foscil}
\end{figure}
