...
resonatorja in stoječa valovanja lastna nihanja. Zaradi dušenja so frekvence lastnih nihanj
razširjenje in imajo končno frekvenčno oziroma spektralno 
...
katere ne obstajajo preproste rešitve v obliki Gaussovih snopov. Taki resonatorji 
...
je intenziteta svetlobe razmeroma velika v celotnem resonatorju in ne le ob osi.  
\end{remark}
...
Označimo jakost električnega polja v točki $P_{1}$ na prvem zrcalu z $E_{1}$ (slika~\ref{fig:uklon_res_shema}).
...
kot med zveznico in normalo na zrcali v osi. Integral prepišemo v 
\begin{equation}
E_2 =  \int_{1}E_{1}(P_{1})K(P_{1},P_{2})dS_{1},
\end{equation}
pri čemer smo vpeljali faktor
\begin{equation}
...
\begin{equation}
\ddot{f} + \omega_{n}^{2}f=0,
\label{3.32}
\end{equation}
kjer smo z dvema pikama označili drugi odvod po času.
