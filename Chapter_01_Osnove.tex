...
ustrezno popraviti.\footnote{Glej npr. R. Podgornik
in A. Vilfan, {\it Elektromagnetno polje}, DMFA--založništvo (2012).} 
Na meji dielektrika z idealnim prevodnikom (kovino,
...
\vglue-5truemm
\begin{definition}
Pokaži zvezo med gostoto toka $j$ (enačba~\ref{eq:jscal}) in
kom\-plek\-snim vektorjem\index{Gostota energijskega toka} $\mathbf{S}(\mathbf{r})$
\begin{equation}
\label{eq:jReS}
j=\left\langle \mathbf{\mathbf{\hat{n}}\cdot\mathbf{S}}\right\rangle =\mathbf{\hat{n}}\cdot
\left(\mathbf{E}\times\mathbf{H}^{*}+\mathbf{E}^{*}\times\mathbf{H}\right)/4=\mathbf{\hat{n}}\cdot 
\Re(\mathbf{S}(\mathbf{r})).
\end{equation}
\end{definition}
...
snovi, je enaka gostoti energijskega oziroma svetlobnega toka $j$\index{Gostota energijskega toka}
\boxeq{eq:jcw}{
...
\end{array}\right].
\end{equation}
Če pa je polarizator zasukan pod kotom $45~\si{\degree}$ v pozitivni 
smeri glede na os $x$, je njegova Jonesova matrika
\begin{equation}
...
Koeficiente $r$ in $t$ izračunamo iz robnih pogojev (enačbe~\ref{eq:robni-pogoji} in \ref{eq:robni-pogoji5}). 
...
\begin{equation}
r_{\mathrm{TM}}=\frac{n_{2}\cos\vartheta_{1}-n_{1}\cos\vartheta_{2}}{n_{2}\cos\vartheta_{1}+n_{1}\cos\vartheta_{2}}
\quad \mathrm{in} \quad 
...
lomni količnik\index{Lomni količnik!redni}, in $n_{3}=n_{e}$, ki je izredni 
(\textit{extraordinary}) lomni količnik\index{Lomni količnik!izredni}\footnote{
