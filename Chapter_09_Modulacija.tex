...
Ko priključimo \index{Dielektričnost!inverzna}
...
zato lahko  
enačbi~(\ref{eq:eonxnyr}) razvijemo:
\begin{equation}
...
\label{7.17}
\end{equation}
Preoblikujemo izraz in zapišemo prepustnost modulatorja ob upoštevanju 
zveze~(\ref{phiAM}):
\boxeq{AMfinal}{
...
odvisnosti od faznega zamika $\Delta \phi$, 
sorazmernega priključeni napetosti (rdeča črta). 
...
\omega_0 + \Delta \omega.
\end{equation}
Razvidno je, da linearno spreminjajoča se modulacijska napetost pomeni spremembo frekvence svetlobe. 
...
\end{equation}
pri čemer sta $\underline{\tilde{b}}$ tenzor v odsotnosti mehanske deformacije in
...
Za čim učinkovitejše delovanje akusto-optičnega modulatorja seveda
...
Vstavimo še parameter $\xi$, ki je podan z enačbo~(\ref{7.38b}).
...
(2016).}\index{Gostota energijskega toka}:
\begin{equation}
...
\label{7.53}
\end{equation}
Upoštevamo izraza za $\beta_m$ in $\xi$~(enačbi~\ref{7.38} in \ref{7.38b}). 
...
v ravnini $xy$. Kot med optično osjo in osjo $x$ zapišemo:
\begin{equation}
\varphi=qz.
\label{7.58}
\end{equation}
