...
\end{equation}
Če vpeljemo odzivni čas $\tau$ (enačba~\ref{TermD_t}), \index{Detektor!odzivni čas}dobimo
\begin{equation}
...
upornost se spreminja s temperaturo.}, \index{Termistor} polprevodnika ali superprevodnika (sliki~\ref{fig:Bolometer-shema} in \ref{fig:Bolometer}). Tipalo prek
...
\caption{Shema bolometra. Počrnjena plast se zaradi vpadne svetlobe segreje, pri čemer
...
sta $R$ upornost sistema in $C_e$ električna kapaciteta detektorja. Občutljivost detektorja, 
...
$\Lambda$ in toplotna kapaciteta detektorja $C$, navzgor odziv omejuje odzivni čas elektronskega vezja $\tau_e$.}
\label{fig:Piro}
\end{figure}
...
\caption{Kvantni izkoristek fotocelic za različne snovi. \index{InGaAs}\index{GaAsP}Povzeto po Hamamatsu Photonics.}
\label{fig:Fotodioda}
\end{figure}
...
\label{FP_j}
\end{equation}
V stacionarnem stanju se število nosilcev naboja ne spreminja. Za gostoto vrzeli velja 
(enačba~\ref{11:eta})
\begin{equation}
0 = \frac{dn_v}{dt} = \frac{\eta_v P}{h \nu Sl} - \frac{\Delta n_v}{\tau_v}
\end{equation}
in podobno za gostoto elektronov $n_e$, le oznake $v$ zamenjamo za $e$. 
Pri tem sta $\eta_v$  in $\eta_e$ kvantna izkoristka, $P$ moč vpadne svetlobe,
$Sl$ prostornina detektorja ter $\tau_v$ in $\tau_e$ življenjski čas vrzeli oziroma elektrona. 
...
\newpage
Koeficient $G$ opiše ojačenje signala.\index{Notranje ojačenje} Njegova vrednost je odvisna od 
...
polprevodniška}}\index{Fotodioda!polprevodniška}
...
rekombinacije in zmanjšanja kvantnega izkoristka (slika~\ref{11_odziv}). 
...
ali fotoprevodnikov\index{Fotoprevodnik} (za IR valovanje),\index{Infrardeče valovanje}
...
in enkrat jih vpade malo več, \index{Šum!štetja}
...
saj je pri detekciji ključno razmerje med signalom in šumom.\index{Razmerje signal proti šumu} 
...
\index{Temni tok}To je tako imenovani temni tok, do katerega pride zaradi
...
moč, ki ustreza šumu).\index{NEP@$NEP$} Gre za vpadno moč svetlobe, ki je po velikosti primerljiva 
